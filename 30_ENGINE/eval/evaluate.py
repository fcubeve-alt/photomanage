...
def report(truth, predicted, catalog, stats, scores, ablation, out) -> int:
    ...
    w("# FIXTURE CONFORMANCE — 10,000-asset SYNTHETIC library\n\n")
    w("> ## ⚠️ These are not product accuracy numbers, and must never be quoted as "
      "such.\n>\n"
      "> Every asset below was **generated by `20_TIER0/study_assets/"
      "generate_test_library.py`**, and the rules being scored were written by someone "
      "who could read that generator. A high score here says the engine reproduces the "
      "generator's own rules. It does not say what happens to a real photograph.\n>\n"
      "> A third-party audit made this point on 2026-09-11 and it is right. What a "
      "real library has that this one does not: forwarded and re-compressed images, "
      "screenshots of screenshots, documents photographed at an angle in bad light, "
      "mixed-language and handwritten OCR, edited and filtered photos, family members "
      "who look alike, and a category distribution nobody designed.\n>\n"
      "> This file was titled *CLASSIFIER EVALUATION* until 2026-09-11, which invited "
      "exactly the misreading. It is a **conformance** result: it proves the Python "
      "engine and its Swift port behave as specified on a known input, which is worth "
      "having and is a different claim.\n\n")
    ...
    for root in SCORED_ROOTS:
        ...
        w(f"| {root} | {c['tp'] + c['fn']:,} | {p:.3f} | {r:.3f} | {f:.3f} | "
          f"{c['tp']:,} | {c['fp']:,} | {c['fn']:,} |\n")
    w(f"\n**Fixture macro F1 across scored roots (synthetic — not product accuracy): {sum(macro)/len(macro):.3f}**\n\n")
