#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
T0-B prototype — the Visual Library home and its drill-down.

DIRECTIVE (Owner, 2026-08-22 · DEC-017):
    This is NOT an "AI work report dashboard". It is first and foremost the entry
    point that replaces Apple Photos, and only second a report of what the system
    did. ORGANIZED is not a handful of smart collections — it is a stable,
    predictable Library Taxonomy that drills down through second and third levels.

Two immediate values on first open (Constitution §12 + §22/§23):
    1. a catalogued library you can browse right now
    2. evidence that the system already did a large amount of the work

SCOPE BOUNDARY — read before extending this:
    What is built here is a NAVIGATIONAL SKELETON, hand-authored for the test
    library. It is NOT the Tier 1-A Visual Asset Taxonomy (classification rules,
    confusion matrices, risk/lifecycle defaults), which the Execution Index forbids
    building now (DEC-003, P-01). Same principle as the hand-prepared catalogue in
    T0-B limitation L-1: we are testing whether the STRUCTURE works for users, not
    whether a classifier can produce it.

Generates:
    prototype/index.html                     home
    prototype/<path>.html                    every category node, to leaf level
    prototype/ledger_v1..v4.html             T0-D header variants of the same home

Usage:  python build_prototype.py
"""

import os, json, html

OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "prototype")

# ---------------------------------------------------------------------------
# THE LIBRARY TAXONOMY SKELETON
# 11 top-level entries per Constitution §23. Depth is what separates a library
# from a pile of smart collections: the user must be able to form a spatial
# memory — "my passport is in Documents > Identity > Passports".
# ---------------------------------------------------------------------------
TAXONOMY = {
  "Documents": {"icon": "📄", "n": 604, "children": {
      "Identity": {"n": 41, "children": {
          "ID Cards":        {"n": 14},
          "Passports":       {"n": 9},
          "Driver Licenses": {"n": 11},
          "Visas":           {"n": 7},
      }},
      "Contracts": {"n": 68, "children": {
          "Tenancy":    {"n": 22},
          "Employment": {"n": 15},
          "Insurance":  {"n": 19},
          "Other":      {"n": 12},
      }},
      "Financial": {"n": 96, "children": {
          "Bank":       {"n": 38},
          "Tax":        {"n": 27},
          "Statements": {"n": 31},
      }},
      "Receipts":  {"n": 212, "also_in": "Purchases > Receipts"},
      "Warranty":  {"n": 34,  "also_in": "Purchases > Warranty"},
      "Medical":   {"n": 29, "pending": True},
      "Other Documents": {"n": 124},
  }},
  "People": {"icon": "🧑", "n": 1532, "children": {
      "Anna":   {"n": 421},
      "Ben":    {"n": 388},
      "Chris":  {"n": 246},
      "Dana":   {"n": 190},
      "Groups": {"n": 287},
  }},
  "Screenshots": {"icon": "📱", "n": 2491, "children": {
      "Chat":     {"n": 612},
      "Shopping": {"n": 448},
      "Maps":     {"n": 203},
      "Web":      {"n": 517},
      "Work":     {"n": 289},
      "Temporary": {"n": 331, "children": {
          "Verification Codes": {"n": 148},
          "Pickup Codes":       {"n": 96},
          "Tickets":            {"n": 87},
      }},
      "Errors":   {"n": 91},
  }},
  "Places": {"icon": "📍", "n": 3118, "children": {
      "United Kingdom": {"n": 2402, "children": {
          "London":   {"n": 1876},
          "Brighton": {"n": 526},
      }},
      "Japan": {"n": 716, "children": {
          "Tokyo": {"n": 716},
      }},
  }},
  "Travel": {"icon": "✈️", "n": 716, "children": {
      "Tokyo · Apr 2025":    {"n": 716},
      "Brighton · Aug 2024": {"n": 214},
      "Lisbon · Sep 2023":   {"n": 168},
  }},
  "Objects": {"icon": "🚲", "n": 208, "children": {
      "Bicycle":    {"n": 3},
      "Appliances": {"n": 46},
      "Devices":    {"n": 71},
      "Furniture":  {"n": 39},
      "Other":      {"n": 49},
  }},
  "Purchases": {"icon": "🧾", "n": 498, "children": {
      "Receipts": {"n": 212, "also_in": "Documents > Receipts"},
      "Orders":   {"n": 174},
      "Warranty": {"n": 34,  "also_in": "Documents > Warranty"},
      "Delivery": {"n": 78},
  }},
  "Clothing":  {"icon": "👕", "n": 341, "children": {
      "Tops": {"n": 128}, "Outerwear": {"n": 74},
      "Shoes": {"n": 62}, "Other": {"n": 77},
  }},
  "Work":      {"icon": "💼", "n": 452, "children": {
      "Whiteboards": {"n": 96}, "Meetings": {"n": 141},
      "Documents": {"n": 118}, "Other": {"n": 97},
  }},
  "Downloads": {"icon": "⬇️", "n": 397, "children": {
      "Memes": {"n": 186}, "Wallpapers": {"n": 84}, "Social": {"n": 127},
  }},
  "Timeline":  {"icon": "🗓", "n": 10000, "children": {
      "2026": {"n": 1842}, "2025": {"n": 2915}, "2024": {"n": 2610},
      "2023": {"n": 1788}, "2022": {"n": 845},
  }},
}

SUMMARY = {
    "analyzed": "10,000",
    "handled":  "2,128",
    "review":   "23",
}

HANDLED = [
    ("284",   "expired verification &amp; pickup codes", "cleared"),
    ("1,203", "duplicate downloads",                     "cleared"),
    ("641",   "near-identical burst frames",             "best kept"),
]
PROTECTED = [("12", "documents"), ("3", "IDs"), ("1", "contract")]

CSS = """
*{box-sizing:border-box;margin:0;padding:0}
body{background:#eef0f3;font:16px/1.45 -apple-system,BlinkMacSystemFont,"SF Pro Text",
     "Helvetica Neue",Arial,sans-serif;color:#111;display:flex;justify-content:center;padding:24px 12px}
a{text-decoration:none;color:inherit}
.phone{width:390px;background:#fff;border-radius:38px;overflow:hidden;
       box-shadow:0 12px 40px rgba(0,0,0,.18);min-height:760px}
.status{height:44px;display:flex;align-items:center;justify-content:space-between;
        padding:0 26px;font-size:13px;font-weight:600}
/* ---- first-run summary: compressed into the header so the LIBRARY gets the
   prime real estate. The work report is second, per DEC-017. ---- */
.summary{padding:6px 22px 18px}
.summary h1{font-size:26px;font-weight:700;letter-spacing:-.4px}
.summary .lines{margin-top:7px;font-size:13.5px;color:#5b626b;line-height:1.6}
.summary .lines b{color:#111;font-variant-numeric:tabular-nums}
.summary .need{margin-top:10px;display:inline-block;background:#eef4ff;color:#0b63e5;
     border-radius:20px;padding:6px 13px;font-size:13px;font-weight:600}
/* ---- nav bar for drill-down levels ---- */
.nav{display:flex;align-items:center;gap:10px;padding:10px 18px 6px}
.nav a.back{font-size:15px;color:#0b63e5;font-weight:600}
.nav .crumb{font-size:12px;color:#8a8f98;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.title{padding:2px 22px 12px}
.title h2{font-size:24px;font-weight:700;letter-spacing:-.3px}
.title .c{font-size:13px;color:#8a8f98;margin-top:2px}
.lbl{font-size:11px;font-weight:700;letter-spacing:1.1px;color:#8a8f98;text-transform:uppercase;
     margin:14px 22px 9px;display:flex;justify-content:space-between;align-items:baseline}
.lbl .foot{font-weight:500;letter-spacing:.2px;text-transform:none;font-size:11px;color:#9aa0a8}
/* ---- library grid: the main event ---- */
.grid{display:grid;grid-template-columns:1fr 1fr;gap:9px;padding:0 22px}
.card{background:#f5f6f8;border-radius:14px;padding:13px 12px;display:block;
      border:1px solid transparent}
.card:hover{border-color:#d9dde3;background:#f0f2f5}
.card .ic{font-size:20px}
.card .nm{font-size:14.5px;font-weight:650;margin-top:6px}
.card .ct{font-size:12px;color:#8a8f98;margin-top:1px;font-variant-numeric:tabular-nums}
/* ---- list rows for deeper levels ---- */
.list{padding:0 22px}
.row{display:flex;align-items:center;gap:12px;padding:13px 2px;border-bottom:1px solid #f0f1f3}
.row:last-child{border-bottom:none}
.row .nm{flex:1;font-size:15px;font-weight:550}
.row .ct{font-size:13px;color:#8a8f98;font-variant-numeric:tabular-nums}
.row .ch{font-size:16px;color:#c3c8cf}
.row .tag{font-size:10.5px;color:#8a6f2e;background:#f7f4ee;border-radius:5px;padding:2px 6px}
.row .pend{font-size:10.5px;color:#a33;background:#fdeaea;border-radius:5px;padding:2px 6px}
/* ---- leaf asset grid ---- */
.assets{display:grid;grid-template-columns:1fr 1fr 1fr;gap:3px;padding:0 22px}
.asset{aspect-ratio:1;border-radius:5px;display:flex;align-items:flex-end;justify-content:center;
       font-size:9px;color:#fff;padding-bottom:4px;text-shadow:0 1px 2px rgba(0,0,0,.5)}
/* ---- work report: BELOW the library ---- */
.hr{height:9px;background:#f1f3f5;margin-top:26px}
.wrow{display:flex;align-items:center;gap:11px;padding:10px 22px;border-bottom:1px solid #f4f5f7}
.wrow .n{font-size:16px;font-weight:700;min-width:50px;font-variant-numeric:tabular-nums}
.wrow .d{flex:1;font-size:13.5px;color:#33383f}
.wrow .s{font-size:11.5px;color:#0a7d3f;background:#e6f6ec;padding:3px 8px;border-radius:20px;
         white-space:nowrap;font-weight:600}
.wrow .why{font-size:11.5px;color:#2f6fd0;white-space:nowrap}
.prot{margin:0 22px;background:#f7f4ee;border-radius:13px;padding:12px 14px}
.prot .l{font-size:13.5px;font-weight:600;color:#7a5c14}
.prot .m{font-size:12px;color:#8a6f2e;margin-top:2px}
.cta{margin:16px 22px 24px;background:#0b63e5;color:#fff;border-radius:15px;padding:14px 17px;
     display:flex;align-items:center;justify-content:space-between}
.cta .l{font-size:15px;font-weight:600}
.cta .r{font-size:14px;opacity:.95}
.note{margin:0 22px 20px;font-size:11.5px;color:#9aa0a8;text-align:center}
.caption{width:390px;text-align:center;color:#8a8f98;font-size:12px;margin-top:14px}
"""

def esc(s): return html.escape(str(s))

def slug(parts):
    return "_".join(p.lower().replace(" ", "-").replace(">", "").replace("·", "")
                     .replace("&", "and").strip("-") for p in parts) or "index"

def shell(body, caption=""):
    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Visual Library</title><style>{CSS}</style></head><body>
<div><div class="phone">
<div class="status"><span>9:41</span><span>&#9679;&#9679;&#9679; &#9646;</span></div>
{body}
</div><div class="caption">{caption}</div></div></body></html>"""

def summary_header(variant="v1"):
    """First-run result, compressed into the header. Library gets the space below."""
    if variant == "v2":                       # no numbers
        return ('<div class="summary"><h1>Your library is organised.</h1>'
                '<div class="lines">Everything analysed.<br>Duplicates, expired codes and '
                'burst frames handled.</div>'
                '<div class="need">A few items need you &rsaquo;</div></div>')
    return (f'<div class="summary"><h1>Your library is organised.</h1>'
            f'<div class="lines"><b>{SUMMARY["analyzed"]}</b> photos and videos analysed<br>'
            f'<b>{SUMMARY["handled"]}</b> items organised or handled</div>'
            f'<div class="need">Only <b>{SUMMARY["review"]}</b> need your attention &rsaquo;</div></div>')

def library_grid():
    cells = []
    for name, node in TAXONOMY.items():
        cells.append(
            f'<a class="card" href="{slug([name])}.html">'
            f'<div class="ic">{node["icon"]}</div>'
            f'<div class="nm">{esc(name)}</div>'
            f'<div class="ct">{node["n"]:,}</div></a>')
    return f'<div class="lbl">Your library</div><div class="grid">{"".join(cells)}</div>'

def work_report(variant="v1"):
    if variant == "v3_no_protected":
        prot = ""
    else:
        items = " · ".join(f"{n} {t}" for n, t in PROTECTED)
        prot = (f'<div class="lbl">Protected</div><div class="prot"><div class="l">{items}</div>'
                f'<div class="m">Never touched automatically</div></div>')
    if variant == "v2":
        rows = "".join(f'<div class="wrow"><div class="d">{d}</div>'
                       f'<div class="s">{s}</div><div class="why">why?</div></div>'
                       for _, d, s in HANDLED)
    else:
        rows = "".join(f'<div class="wrow"><div class="n">{n}</div><div class="d">{d}</div>'
                       f'<div class="s">{s}</div><div class="why">why?</div></div>'
                       for n, d, s in HANDLED)
    return ('<div class="hr"></div>'
            '<div class="lbl">Handled for you <span class="foot">all recoverable for 30 days</span></div>'
            + rows + prot)

def review_cta():
    return (f'<a class="cta" href="#"><div class="l">{SUMMARY["review"]} items need you</div>'
            f'<div class="r">Review &rsaquo;</div></a>')

# ---------------------------------------------------------------------------

pages = {}

def build_node(path, node):
    """Recursively emit a page per taxonomy node."""
    name = path[-1]
    children = node.get("children")
    parent_href = (slug(path[:-1]) + ".html") if len(path) > 1 else "index.html"
    crumb = " › ".join(path)
    nav = (f'<div class="nav"><a class="back" href="{parent_href}">&lsaquo; Back</a>'
           f'<div class="crumb">{esc(crumb)}</div></div>')
    head = (f'<div class="title"><h2>{esc(name)}</h2>'
            f'<div class="c">{node["n"]:,} items</div></div>')

    if children:
        rows = []
        for cname, cnode in children.items():
            href = slug(path + [cname]) + ".html"
            tag = ""
            if cnode.get("also_in"):
                tag = f'<span class="tag">also in {esc(cnode["also_in"])}</span>'
            if cnode.get("pending"):
                tag = '<span class="pend">pending decision</span>'
            rows.append(
                f'<a class="row" href="{href}"><div class="nm">{esc(cname)}</div>{tag}'
                f'<div class="ct">{cnode["n"]:,}</div><div class="ch">&rsaquo;</div></a>')
            build_node(path + [cname], cnode)
        body = nav + head + f'<div class="list">{"".join(rows)}</div>'
    else:
        # Leaf: the assets themselves. Structure First, Photos Second (§23) —
        # photos appear only once the user has arrived somewhere specific.
        n = min(node["n"], 18)
        tiles = []
        for i in range(n):
            h = (hash(name) + i * 977) % 360
            tiles.append(f'<div class="asset" style="background:hsl({h},42%,58%)">{esc(name)}</div>')
        body = nav + head + f'<div class="assets">{"".join(tiles)}</div>'

    pages[slug(path) + ".html"] = shell(
        body, f"T0-B prototype · {esc(crumb)}")

def build_home(filename, variant="v1", review_first=False):
    parts = [summary_header(variant)]
    if review_first:
        parts.append(review_cta())          # V4: homework above the library
        parts.append(library_grid())
        parts.append(work_report(variant))
    else:
        parts.append(library_grid())        # library first, always
        parts.append(work_report(variant))
        parts.append(review_cta())
    cap = "T0-B prototype · Home" if variant == "v1" and not review_first \
          else f"T0-D variant · {filename.replace('.html','')}"
    pages[filename] = shell("".join(parts), cap)


def main():
    os.makedirs(OUT, exist_ok=True)
    for name, node in TAXONOMY.items():
        build_node([name], node)

    build_home("index.html", "v1")
    build_home("ledger_v1.html", "v1")
    build_home("ledger_v2.html", "v2")
    build_home("ledger_v3.html", "v3_no_protected")
    build_home("ledger_v4.html", "v1", review_first=True)

    for fn, htm in pages.items():
        with open(os.path.join(OUT, fn), "w", encoding="utf-8") as f:
            f.write(htm)

    depth = max(len(k.split("_")) for k in pages)
    print(f"wrote {len(pages)} pages -> {OUT}")
    print(f"max taxonomy depth: {depth} levels")
    print("home: index.html   ·   T0-D variants: ledger_v1..v4.html")
    print("\ncheck: Documents > Identity > Passports =",
          "documents_identity_passports.html" in pages)


if __name__ == "__main__":
    main()
