...
| `build_ledger_variants.py` | Work-Done Ledger V1–V4, generated from one template |
| `ledger/v1_full.html` … `v4_review_first.html` | the four T0-D variants |
...
The ledger variants are generated from a single template on purpose: a between-subjects manipulation is only valid if the variants are identical except for the manipulated dimension. Hand-editing four HTML files would let wording and spacing drift and silently confound the result. If the library size changes, edit `LIBRARY` in the script and regenerate — never edit the HTML by hand.
