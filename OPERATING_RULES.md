...
- **P-04 On-device AI route is decided by benchmark, not by name.** **Restated 2026-08-24 per the Architecture Methodology:** the question is not *which model is strongest* but **what is the minimum computational capability required for this specific task**. Rules/metadata solve it → use no AI. A light model solves it → do not use a large one. Apple Native solves it → add no model. Only a benchmarked capability gap justifies researching Gemma / Mage / LocateAnything / grounding-mini, and any added model must show its gain outweighs binary size, memory, battery, thermal cost and engineering complexity. **LocateAnything, Mage and Gemma are Candidate/Benchmark status — never product dependencies.**
  - **Tightened 2026-08-22:** there is now a shipped existence proof that a full on-device stack (NL search, OCR, object/face recognition, embeddings, smart collections) fits in **262 MB** (Lucent Pro), and comparable search in 206 MB (Queryable). A heavy model must therefore **beat a demonstrated 262 MB baseline**, not merely work. Evidence: `20_TIER0/evidence/LUCENT_PRO_BENCHMARK_COMPETITOR.md` P-1.
...
- **P-08 Medical means documents, never diagnoses.** `Documents > Medical` may classify an explicit medical **document** the user photographed — a prescription, a test result, an appointment letter. It may **never** infer, store, display or act on a health **state**: no condition, diagnosis, medication, treatment or severity, and no derived signal about the person. The category describes the piece of paper, never the human. Contents default to **Protect** (R5-class, §6). This boundary is one careless model call away from being crossed, so any future classifier touching this category needs an explicit review against this rule. Source: Constitution §16 + Owner ruling DEC-020.
- **P-09 Minimum Necessary Inference (MNI).** Mandatory order: **Cheap Signals → Candidate Reduction → Selective Intelligence → Structured Memory.** Handing every asset to a heavy model is wrong *by construction*, not merely slow. Cheap signals first: metadata, hash/pHash, OCR, light embeddings, Apple Vision, the existing index, and for video the codec / scene-change / motion information. The expensive layer is entered only where cheap signals cannot reliably finish the task. **The architecture never bends to accommodate a model.** Source: `ARCHITECTURE_METHODOLOGY.md`.
- **P-10 Information-Change-First (ICF).** Before any expensive inference, answer in order: *What changed? What is actually new? Does this new information deserve deeper inference?* Near-duplicates, same-moment bursts and identical re-downloads must not be re-understood as independent new objects. **Video is forbidden from defaulting to frame-sampling + full model per frame**; it must go codec/metadata → scene & change detection → meaningful segments → selective understanding → **Video Memory Record**, into the same Unified Visual Memory Graph as photos. Source: `ARCHITECTURE_METHODOLOGY.md`.
