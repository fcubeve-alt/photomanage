...
> 🛑 **CI IS DOWN — GitHub Actions minutes are exhausted (2026-09-06).**
> Evidence, not inference: four consecutive jobs — three `macos-15`, one `ubuntu-latest` — failed in 3-7 seconds with **no runner assigned and no logs at all**, immediately after runs on the same configurations had worked. That is runner allocation being refused, which on a private repository means the Actions budget.
> **It is not macOS-specific.** Linux fails the same way, so moving work to cheaper runners does not restore verification.
> **Consequence:** ~4,100 lines of Swift in `40_APP/` are **unverified** — they have never compiled. The Python engine in `30_ENGINE/` is unaffected and still verifies locally (`python -m unittest discover -s tests`, `python check_constraints.py`).
> **Owner options:** (a) wait for the monthly reset, (b) raise the spending limit under GitHub → Settings → Billing, (c) make the repository public, where Actions is free — that publishes everything, so it is a decision, not a workaround. Installing a Swift toolchain in the session is not an option either: `download.swift.org` is denied by the egress policy.
