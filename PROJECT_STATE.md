...
- Repo: `github.com/fcubeve-alt/photomanage` · **PUBLIC** (verified against the GitHub API 2026-09-11) · active branch **`claude/classification-program-dev-yk88jj`**, which is 67 commits ahead of `main`
- ⚠️ **The repo is PUBLIC and a decision below still says it must be PRIVATE.** See DEC-033. The likely cause is the CI-minutes decision recorded in the 2026-09-06 handoff, which listed "make the repository public, where Actions is free" as one of three options — taken, but never written down. Forks 0, stars 0, watchers 0 at the time of checking. This needs an Owner ruling either way; nothing has been changed.
...
- **GitHub Actions is reachable from the working session** via the GitHub MCP tools — workflows have been dispatched and their logs read repeatedly. `gh` may still be absent on the Owner's own Windows machine; that is a statement about that machine, not about this project, and it was blocking nothing.
...
## Done
- **First measured Tier 0-A data exists** (2026-09-06). Scale sweep on the iOS Simulator, $0, HG-1 not required: **105–154 ms/asset over two runs of identical code**, linear within each run. Ceiling **35k–52k** assets in the 90-min foreground budget, **187k–275k** in the 8-h background budget; predicted device band **7k–26k** foreground (PREDICTION, no device has run this). Quote it as a band: the 1.47x run-to-run spread is shared-runner CPU contention, and every deterministic output matched exactly across both runs. Evidence: `20_TIER0/evidence/T0A_SCALE_SIMULATOR_2026-09-06.md`. **Still not an A1–A4 verdict** — no thermal, no jetsam, no battery, no Neural Engine, no real library.
- **The classification engine is built** (DEC-028, Owner instruction). `30_ENGINE/` — signal contract, escalating classifier, R0–R6 risk policy, duplicate / same-entity handling, SQLite catalogue with checkpoint-resume and incremental update, CLI, 55 tests, evaluation harness. Measured on the 10,000-asset labelled library: root macro **F1 0.998** *(fixture conformance on a synthetic library — not product accuracy; see the warning at the top of the evaluation report)*, leaf exact **99.8%** where derivable, **0** wrong roots, 4/4 safety red lines audited. **PF-11 still applies to everything else** — this is one engine, not a product.
...
6. ~~**T0-A-HARNESS** Swift benchmark harness~~ — ✅ **DONE** → `20_TIER0/harness/` (13 files, ~1,400 lines). The old note "**NEVER COMPILED**" was true when written and stopped being true on 2026-09-06: it compiles and its 17 tests run on the Simulator (run 33984153562).
...
- **~~Repo must be PRIVATE.~~ SUPERSEDED IN FACT, NOT IN DECISION — see DEC-033.** The repository is public as of 2026-09-11. The reasoning below still stands on its merits and was never formally revoked, which is the problem: the Constitution, the pricing strategy, the $39 test and the unlaunched landing copy are all public now. Original text: *Free minutes are not a reason to go public: ~150 bring-up minutes fit inside the 2,000/mo Free allowance either way, so private is free for our volume. Publishing would expose the Constitution, pricing strategy, the $39 test and the unlaunched landing copy.*
